...
var xmldom = require("./xmldom/dom-parser.js"); // //console.log('xml', xmldom, typeof xmldom.DOMParser);
...
var window = window || {};
window.DOMParser = DOMParser; //console.log('strophe ...')
...
(function (callback) {
  ...
  (function (root, factory) {
    if (typeof define === 'function' && define.amd) {
      ...
      root.str_sha1 = o.SHA1.str_sha1; //console.log(root);
    ...
    Strophe.Connection = function (service, options) {
      ...
      for (var k in Strophe._connectionPlugins) {
        if (Strophe._connectionPlugins.hasOwnProperty(k)) {
          var ptype = Strophe._connectionPlugins[k]; // jslint complaints about the below line, but this is fine
    ...
    Strophe.Connection.prototype = {
      ...
      *  > };
      ...
      sendIQ: function (elem, callback, errback, timeout) {
        ...
        var id = elem.getAttribute('id'); // inject id if not found
        ...
        }, null, 'iq', ['error', 'result'], id); // if timeout specified, setup timeout handler.
      ...
      _sendRestart: function () {
        ...
        this._proto._sendRestart(); // XXX: setTimeout should be called only with function expressions (23974bc1)
      ...
      _dataRecv: function (req, raw) {
        ...
        Strophe.forEachChild(elem, null, function (child) {
          var i, newList; // process handlers

          newList = that.handlers;
          that.handlers = [];

          for (i = 0; i < newList.length; i++) {
            var hand = newList[i]; // encapsulate 'handler.run' not to lose the whole handler list if
      ...
      _sasl_success_cb: function (elem) {
        ...
        streamfeature_handlers.push(this._addSysHandler(function (elem) {
          wrapper.bind(this)(streamfeature_handlers, elem);
        }.bind(this), Strophe.NS.STREAM, "features", null, null)); // we must send an xmpp:restart now
      ...
      _onIdle: function (fail) {
        var i, thand, since, newList; // add timed handlers scheduled for addition
        ...
        this._proto._onIdle(fail); // reactivate the timer only if connected
  ...
  (function (root, factory) {
    ...
    Strophe.Bosh.prototype = {
      ...
      _connect: function (wait, hold, route) {
        ...
        this.errors = 0; // build the body tag
      ...
      _no_auth_received: function (_callback) {
        ...
        this._requests.push(new Strophe.Request(body.tree(), this._onRequestStateChange.bind(this, _callback.bind(this._conn)), body.tree().getAttribute("rid"))); //console.log(this._requests)
      ...
      _send: function (fail) {
        ...
        this._throttledRequestHandler(); // XXX: setTimeout should be called only with function expressions (23974bc1)
  ...
  (function (root, factory) {
    ...
    Strophe.Websocket.prototype = {
        ...
        function creatSocket() {
          ...
          setTimeout(() => {
            var SocketTask = wx.connectSocket({
              ...
              fail: function (e) {
                console.log('连接失败', e); //部分机型从后台切回前台状态有延迟
            ...
            _socketTask.onClose(function (e) {
              ...
              me.socket.onclose.call(me); // 外部回调，需要设计一个更合适的

              me._onSocketClose && me._onSocketClose(e); //me._conn._changeConnectStatus(Strophe.Status.DISCONNECTED, e);
            });
      ...
      _handleStreamStart: function (message) {
        var error = false; //console.log('_handleStreamStart', message, typeof message)
      ...
      _connect_cb_wrapper: function (message) {
        ...
        if (message.data.indexOf("<open ") === 0 || message.data.indexOf("<?xml") === 0) {
          ...
          var data = message.data.replace(/^(<\?.*?\?>\s*)*/, ""); //console.log('_connect_cb_wrapper open 3', data)

          if (data === '') return; //console.log(new DOMParser().parseFromString(data, "text/xml"))
          //console.log('_connect_cb_wrapper open 3.1', data)

          var streamStart = new DOMParser().parseFromString(data, "text/xml").documentElement; //console.log('_connect_cb_wrapper open 4', streamStart)
          ...
          var string = this._streamWrap(message.data); //console.log('_connect_cb_wrapper inited 5')


          var elem = new DOMParser().parseFromString(string, "text/xml").documentElement; //console.log('_connect_cb_wrapper inited 6')

          this.socket.onmessage = this._onMessage.bind(this); //console.log('_connect_cb_wrapper inited 7')
       ...
       */
      _disconnect: function (pres) {
        console.log('_disconnect', this.socket.readyState, !!this.socket); //  && this.socket.readyState !== WebSocket.CLOSED
        ...
        if (data.length > 0 && !this._conn.paused) {
          for (var i = 0; i < data.length; i++) {
            if (data[i] !== null) {
              ...
              this._conn.rawOutput(rawStanza); // onsend todo
              ...
              if (isSocketConnnected) {
                ...
                //this._conn._doDisconnect();
                isSocketConnnected = false;
                fail && fail('socket disconnnected'); //this._changeConnectStatus(Strophe.Status.DISCONNECTED, null);
      ...
      _onOpen: function () {
        ...
        this._conn.rawOutput(startString); // onsend todo
...
}).bind(window)(function (Strophe, build, msg, iq, pres) {// window.Strophe = Strophe;
