...
Strophe.log = function (level, msg) {// console.log(ts(), level, msg);
};

let xmldom = require("./libs/xmldom/dom-parser.js"); // //console.log('xml',xmldom, typeof xmldom.DOMParser);
...
let _code = require("./status.js").code;

let _utils = require("./utils.js").utils;

let _msg = require("./message.js");
...
Strophe.Websocket.prototype._onSocketClose = function (e) {// if(e.code && e.code == 1000){
  //
  // }
  // else{
  // 	reOpenEntry();
...
Strophe.Websocket.prototype._onMessage = function (message) {
  var elem, data; // WebIM && WebIM.config.isDebug && //console.log(WebIM.utils.ts() + 'recv:', message.data);
  ...
  } catch (e) {} // console.log('%crecv' + message.data, 'color: green');
...
function _parseResponseMessage(msginfo) {
  ...
  var msgBodies = msginfo.getElementsByTagName("body"); // //console.log('msginfo', msgBodies)
...
function _parseFriend(queryTag, conn, from) {
  ...
  if (items) {
    for (let i = 0; i < items.length; i++) {
      ...
      rouster.push(friend); // B 同意之后 -> B 订阅 A
...
function _loginCallback(status, msg, conn) {
  ...
  if (status == Strophe.Status.CONNFAIL) {
    ...
    conn.autoReconnectNumTotal = 0; // client should limit the speed of sending ack messages  up to 5/s
    ...
    conn.addHandler(handlePresence, null, "presence", null, null, null); // conn.addHandler(handlePing, "urn:xmpp:ping", "iq", "get", null, null);
    ...
    let supportRecMessage = [_code.WEBIM_MESSAGE_REC_TEXT, _code.WEBIM_MESSAGE_REC_EMOJI];
    let supportSedMessage = [_code.WEBIM_MESSAGE_SED_TEXT];
...
function connection(options) {
  ...
  this.apiUrl = options.apiUrl || ""; // todo 接收的事件，放到数组里的时候，加上g.isInBackground字段。每帧执行一个事件的时候，如果g.isInBackground=true,就pass
...
connection.prototype.listen = function (options) {
  ...
  this.onCreateGroup = options.onCreateGroup || _utils.emptyfn; // for WindowSDK
...
connection.prototype.open = function (options) {
  let me = this;
  console.log("open", this.isOpening()); // 防止重复初始化
...
connection.prototype.handlePresence = function (msginfo) {
  ...
  if (members && members.length > 0) {
    ...
    let affiliation = member.getAttribute("affiliation"); // dismissed by group
  ...
  if (x && x.length > 0) {
    // 加群申请
    let apply = x[0].getElementsByTagName("apply"); // 加群成功

    let accept = x[0].getElementsByTagName("accept"); // 同意加群后用户进群通知

    let item = x[0].getElementsByTagName("item"); // 加群被拒绝

    let decline = x[0].getElementsByTagName("decline"); // 被设为管理员

    let addAdmin = x[0].getElementsByTagName("add_admin"); // 被取消管理员

    let removeAdmin = x[0].getElementsByTagName("remove_admin"); // 被禁言

    let addMute = x[0].getElementsByTagName("add_mute"); // 取消禁言
  ...
  if (info.chatroom) {
    ...
    } else if (type == "" && !info.status && !info.error && !isCreate && !isApply && !isMemberJoin && !isDecline) {} // info.type = 'joinPublicGroupSuccess';
...
connection.prototype.handleIqRoster = function (e) {
  ...
  var from = e.getAttribute("from") || ""; // var name = _parseNameFromJidFn(from);

  var curJid = this.context.jid; // var curUser = this.context.userId;
...
connection.prototype.handleMessage = function (msginfo) {
  ...
  let id = msginfo.getAttribute("id") || ""; // cache ack into sendQueue first, handelSendQueue will do the send thing with the speed of 5/s

  this.cacheReceiptsMessage({
    id: id
  }); // console.log('handlePresence', msginfo)

  let parseMsgData = _parseResponseMessage(msginfo); // console.log('parseMsgData', parseMsgData)
  ...
  if (error.length > 0) {
    ...
    errorText = textDOM[0].textContent || textDOM[0].text; // log("handle error", errorCode, errorText);
  }
...
connection.prototype.send = function (message) {
  if (WebIM.config.isWindowSDK) {
    WebIM.doQuery(JSON.stringify({
      type: "sendMessage",
      to: message.to,
      message_type: message.type,
      msg: encodeURI(message.msg),
      chatType: message.chatType
    }), function (response) {}, function (code, msg) {});
...
connection.prototype.changeGroupSubject = function (options) {
  ...
  }).c("value").t(options.subject).up().up().c("field", {
    "var": "muc#roomconfig_roomdesc"
  }).c("value").t(options.description);
  this.context.stropheConn.sendIQ(iq.tree(), function (msginfo) {
    sucFn();
  }, function () {
    errFn();
